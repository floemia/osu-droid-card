...
// src/DroidCard/DroidCard.ts
import { createCanvas as createCanvas2, loadImage, GlobalFonts } from "@napi-rs/canvas";
...
var CanvasHelper = class {
  ...
  static blurRoundedRegion(ctx, x, y, width2, height2, radius, blur) {
    const offCanvas = createCanvas(width2, height2);
    const offCtx = offCanvas.getContext("2d");
    const imageData = ctx.getImageData(x, y, width2, height2);
    offCtx.putImageData(imageData, 0, 0);
    offCtx.filter = `blur(${blur}px)`;
    const blurredCanvas = createCanvas(width2, height2);
  ...
  static blurImage(image, radius) {
    ...
    const canvas = createCanvas(width2 - radius * 2, height2 - radius * 2);
    const ctx = canvas.getContext("2d");
    const temp_canvas = createCanvas(width2, height2);
...
var DroidCard = class {
  ...
  static async getCardBuffer(user, rx) {
    ...
    const canvas = createCanvas2(mt.w, mt.h);
    const ctx = canvas.getContext("2d");
    ...
    for (const score of scores) {
      ...
      let bg2;
      try {
        bg2 = await loadImage(bg_url);
      } catch {
        bg2 = await loadImage(filepath.bg);
      }
      const r = mt.rad;
      let radius = [r, r, r, r];
      if (scores.length > 1) {
