...
var card = async (params) => {
  if (!params.user) {
    if (!params.uid)
      return void 0;
    const data = await import_osu_droid_scraping.droid.request(params.uid);
    if (!data)
      return void 0;
    params.user = await import_osu_droid_scraping.droid.user({ uid: params.uid, response: data });
    params.scores = await import_osu_droid_scraping.droid.scores({ uid: params.uid, type: "top", response: data });
  }
  if (!params.scores)
    return void 0;
  const canvas = (0, import_canvas.createCanvas)(1100, 650);
  const ctx = canvas.getContext("2d");
  let dir;
  try {
    dir = __dirname;
  } catch (e) {
    dir = (0, import_path2.dirname)((0, import_url.fileURLToPath)(import_meta.url));
  }
  import_canvas.GlobalFonts.registerFromPath(import_path.default.join(dir, `../assets/fonts/SF-Pro-Rounded-Bold.otf`), "sftitle");
  import_canvas.GlobalFonts.registerFromPath(import_path.default.join(dir, `../assets/fonts/SF-Pro-Rounded-Medium.otf`), "sfbody");
  ctx.beginPath();
  ctx.roundRect(0, 0, 1100, 650, 60);
  ctx.closePath();
  ctx.clip();
  const background = await (0, import_canvas.loadImage)(import_path.default.join(dir, `../assets/images/background/bg.png`));
  ctx.drawImage(background, 0, 0, background.width * 1.2, background.width * 1.2);
  const avatar = await (0, import_canvas.loadImage)(params.user.avatar_url);
  const accent = await (0, import_fast_average_color_node.getAverageColor)(params.user.avatar_url);
  ...
  const flag = await (0, import_canvas.loadImage)(flag_url(params.user.country));
  ctx.save();
  ctx.fillStyle = "white";
  ...
  const logo = await (0, import_canvas.loadImage)(import_path.default.join(dir, `../assets/images/logo50px.png`));
  ...
  for (const score of params.scores) {
    ...
    if (beatmap) {
      ...
      try {
        const bg = await (0, import_canvas.loadImage)(cover);
        ctx.drawImage(bg, 570, y, bg.width / 1.5, bg.height / 1.5);
      } catch {
        const bg = await (0, import_canvas.loadImage)(import_path.default.join(dir, `../assets/images/background/default-bg@2x.png`));
        ctx.drawImage(bg, 570, y, bg.width / 1.5, bg.height / 1.5);
      }
      ctx.restore();
      ...
      let avg;
      try {
        avg = await (0, import_fast_average_color_node.getAverageColor)(cover);
      } catch {
        avg = await (0, import_fast_average_color_node.getAverageColor)(import_path.default.join(dir, `../assets/images/background/default-bg.png`));
      }
    ...
    const rank = await (0, import_canvas.loadImage)(import_path.default.join(dir, `../assets/images/ranks/${score.rank}55px.png`));
