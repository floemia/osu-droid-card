...
// src/functions.ts
import { MapInfo as MapInfo2, Accuracy, ModUtil as ModUtil3, OsuAPIRequestBuilder, ModCustomSpeed as ModCustomSpeed4, Modes, ModRelax as ModRelax3 } from "@rian8337/osu-base";
import { getAverageColor as getAverageColor3 } from "fast-average-color-node";
import { DroidLegacyModConverter as DroidLegacyModConverter2 } from "@rian8337/osu-base";
...
import { ModCustomSpeed, ModDoubleTime, ModHalfTime, ModMap, ModNightCore } from "@rian8337/osu-base";
...
// src/Utils/Utils.ts
import { ModCustomSpeed as ModCustomSpeed2, ModUtil } from "@rian8337/osu-base";
var Utils = class {
  static {
    __name(this, "Utils");
  }
  static toModMap(mods2) {
    ...
    const modMap = ModUtil.pcStringToMods(mods2.join(""));
    if (speed != 1) modMap.set(new ModCustomSpeed2(speed));
    return modMap;
  }
};
...
import { getAverageColor as getAverageColor2 } from "fast-average-color-node";
...
var DroidUser = class {
  ...
  static async getColor(avatar_url) {
    if (avatar_url == default_avatar) return "#dedede";
      ...
      try {
        return (await getAverageColor2(avatar_url)).hex;
      } catch {
        return "#dedede";
      }
    }
...
import { ModCustomSpeed as ModCustomSpeed3, ModUtil as ModUtil2 } from "@rian8337/osu-base";
...
var DroidScrape = class _DroidScrape {
  ...
  static parseScores(html) {
    ...
    const scores2 = score_blocks.map((block) => {
      ...
      const mods2 = ModUtil2.pcStringToMods(mods_list.acronyms.join());
      if (mods_list.speed != 1) mods2.set(new ModCustomSpeed3(mods_list.speed));
      return {
        rank,
        filename,
        date: timestamp,
...
var calculate = /* @__PURE__ */ __name(async (score) => {
  ...
  if (!beatmapInfo) {
    const bmap = await MapInfo2.getInformation(score.hash);
    if (!bmap) {
  ...
  if (mods2.has(ModRelax3)) mods2.delete(ModRelax3);
  const droid_rating = new DroidDifficultyCalculator().calculate(beatmapInfo.beatmap, mods2);
  if (isBancho) score.replay = await score.getReplay(droid_rating);
  const osu_rating = new OsuDifficultyCalculator().calculate(beatmapInfo.beatmap, mods2);
  ModUtil3.applyModsToBeatmapDifficulty(beatmapInfo.beatmap.difficulty, Modes.osu, mods2);
...
var performance = /* @__PURE__ */ __name(async (details) => {
  ...
  try {
    const color = await getAverageColor3(`https://assets.ppy.sh/beatmaps/${details.beatmap.beatmapSetId}/covers/card.jpg`);
    color_hex = color.hex;
  } catch {
    color_hex = "#dedede";
  }
  let beatmap = details.beatmap;
  const mods2 = ModUtil3.pcStringToMods(details.mods.acronyms.join());
  if (mods2.has(ModCustomSpeed4)) mods2.set(new ModCustomSpeed4(details.mods.speed));
  ...
  const droid_rating = new DroidDifficultyCalculator().calculate(beatmap.beatmap, mods2);
  const osu_rating = new OsuDifficultyCalculator().calculate(beatmap.beatmap, mods2);
  const osu_performance = new OsuPerformanceCalculator(osu_rating).calculate(perf_stats);
  const droid_performance = new DroidPerformanceCalculator(droid_rating).calculate(perf_stats);
...
var mods = /* @__PURE__ */ __name((droid_mods) => DroidLegacyModConverter2.convert(droid_mods), "mods");
